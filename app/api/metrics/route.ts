...
export async function GET(request: Request) {
  const appVersion = process.env.CLOUDY_APP_VERSION || "development";
  ...
  try {
    ...
    return new Response(await response.text(), {
      headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", "x-cloudy-version": appVersion },
    });
  } catch {
