      ...
      <section className={`pet-widget ${displayMood.id} ${panel !== "closed" ? "expanded" : ""}`} aria-label="奶崽服务器桌面宠物">
        ...
        {panel === "setup" && <section className="drawer setup-drawer">
          ...
          <ol>
            ...
            <li><span>3</span><div><b>在腾讯云创建 SSH 密钥</b><p>进入腾讯云控制台的 SSH 密钥页面，创建并下载私钥，再把密钥绑定到这台服务器。私钥文件只会下载一次，请保存在自己的电脑。</p></div></li>
            <li><span>4</span><div><b>填写服务器并选择私钥</b><p>打开桌面奶崽，填写服务器公网 IP 和 SSH 用户名，再选择刚下载的私钥文件。奶崽会自动读取服务信息，不需要复制配对码或 token。</p><p>奶崽只保存私钥的本地路径，密钥内容不会被复制或上传。</p></div></li>
            <li><span>5</span><div><b>不想养了，一行卸载</b><p>回到服务器的 cloudpet 目录执行，会停止服务并删除服务器应用文件和克隆目录：</p><div className="command"><code>sudo sh uninstall-native.sh && cd .. && rm -rf cloudpet</code><button onClick={() => copy("uninstall", "sudo sh uninstall-native.sh && cd .. && rm -rf cloudpet")}>{copied === "uninstall" ? "好啦" : "复制"}</button></div></div></li>
          </ol>
        </section>}
